...
function waitForSpawn(bot) {
  return new Promise((resolve) => { bot.client.once('spawn', resolve); });
}
const CARDINAL_YAWS = {
  NORTH : 0,
...
async function run(bot, options) {
  ...
  await new Promise(resolve => { bot.client.look(yaw, pitch, true, resolve); });
  await delay(500);
  ...
  await new Promise(
      resolve => { bot.client.lookAt(portalPos, true, resolve); });
  bot.client.setControlState('jump', true);
  await delay(25);
  bot.client.setControlState('sprint', true);
  bot.client.setControlState('forward', true);
...
function wiggle(bot) {
  function moveLeft() { bot.client.setControlState('left', true); }
  function moveRight() { bot.client.setControlState('right', true); }
  let left = true;
  const interval = setInterval(() => {
    if (bot.client == null)
      return;
